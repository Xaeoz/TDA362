...
void main() 
{
	...
	normal = normalize(cross(vector21, vector31));
	viewSpaceNormal = normalize((normalMatrix *  vec4(normal, 0.0)).xyz);
	//viewSpaceNormal = vec3(0.0f, 1.0f, 0.0f);
	//viewSpaceNormal = normalize(viewSpaceNormal);
	gl_Position = modelViewProjectionMatrix * vec4(position.x, 0-texture2D(heightMap, texCoordIn.xy).x/2, position.z, 1.0f);
