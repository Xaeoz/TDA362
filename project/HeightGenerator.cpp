		...
		for (int x = 0; x < rowLength; x++) {
			for (int y = 0; y < rowLength; y++) {
				if (normalizeMode == Local) {
					...
					outputArray[x * rowLength + y] = clamp((toNormalize - minNoise) / (noiseDifference) - falloffMap[x * rowLength + y], 0.f, FLT_MAX) ;
				}
				else {
					float normalizedHeight = (outputArray[x * (rowLength) + y] + 1) / (2.f * maxPossibleHeight / 1.5f);
					outputArray[x * (rowLength)+y] = clamp(normalizedHeight - falloffMap[x * rowLength + y], 0.f, FLT_MAX);
				}
			}
		}
